...
/*
        ...
        public static void SendChatTextPost(IntPtr hWnd, string msg) {
            PostMessage(hWnd, new Key(KeyCode.Return));
        ...
        public static void SendChatTextSend(IntPtr hWnd, string msg) {
            SendMessage(hWnd, new Key(KeyCode.Return), true);
            foreach (char c in msg) {
        ...
        public static bool ForegroundKeyPressAll(IntPtr hWnd, Key key, bool alt, bool ctrl, bool shift, int delay = 100) {
            ...
            structInput.ki.dwFlags = 0;
            if (alt) {
                structInput.ki.wVk = (ushort) KeyCode.Menu;
                intReturn = SendInput(1, ref structInput, Marshal.SizeOf(new INPUT()));
                Thread.Sleep(delay);
            }
            if (ctrl) {
                structInput.ki.wVk = (ushort) KeyCode.Control;
                intReturn = SendInput(1, ref structInput, Marshal.SizeOf(new INPUT()));
                Thread.Sleep(delay);
            }
            if (shift) {
                structInput.ki.wVk = (ushort) KeyCode.Shift;
                intReturn = SendInput(1, ref structInput, Marshal.SizeOf(new INPUT()));
                Thread.Sleep(delay);

                if (key.ShiftKey != KeyCode.None) {
            ...
            structInput.ki.dwFlags = KEYEVENTF_KEYUP;
            if (shift && key.ShiftKey == KeyCode.None) {
                structInput.ki.wVk = (ushort) KeyCode.Shift;
                intReturn = SendInput(1, ref structInput, Marshal.SizeOf(new INPUT()));
                Thread.Sleep(delay);
            }
        ...
        public static bool SendPostDown(IntPtr hWnd, Key key, int delay = 100) {
        ...
        public static bool SendPostUp(IntPtr hWnd, Key key, int delay = 100) {
            //Send KEY_UP
            Thread.Sleep(delay);
        ...
        public static void CheckKeyShiftState() {
            while ((GetKeyState((int) KeyCode.Menu) & KEY_PRESSED) == KEY_PRESSED ||
                   (GetKeyState((int) KeyCode.Control) & KEY_PRESSED) == KEY_PRESSED ||
                   (GetKeyState((int) KeyCode.Shift) & KEY_PRESSED) == KEY_PRESSED) {
                Thread.Sleep(1);
            }
        }
        ...
        private static uint GetScanCode(KeyCode key) {
            return MapVirtualKey((uint) key, MAPVK_VK_TO_VSC_EX);
        }

        private static uint GetDwExtraInfo(Int16 repeatCount, KeyCode key, byte extended, byte contextCode, byte previousState,
        ...
        private static uint GetLParam(Int16 repeatCount, KeyCode key, byte extended, byte contextCode, byte previousState,
            byte transitionState) {
            var lParam = (uint) repeatCount;
